...
function restartGame() {
    window.location.href = 'jeu5.html'; // Assurez-vous que le chemin est correct
}

function loadLeaderboard() {
    const leaderboardElement = document.getElementById('leaderboardJeu5');
    firebase.database().ref('/scores').orderByChild('jeu5').limitToLast(10).on('value', (snapshot) => {
      const scores = snapshot.val();
      if (scores) {
        const sortedScores = Object.keys(scores).map(player => ({ player, score: scores[player]['jeu5'] })).sort((a, b) => b.score - a.score);
